...
function CustomerCard({
  ...
  const formattedPhone = customer.phone ? formatPhone(customer.phone) : "Sem telefone cadastrado";
  const toggleLabel = isOpen
    ? `Fechar detalhes de ${customer.name}`
    : `Abrir detalhes de ${customer.name}`;
  ...
  return (
    <article className="rounded-[1.5rem] border border-white/60 bg-gradient-to-br from-white/80 to-white/30 p-4 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md">
      <div className="flex items-start justify-between gap-3">
        ...
        >
          <div className="space-y-1">
            ...
            <p className="text-sm text-zinc-600">{formattedPhone}</p>
          </div>
        </button>
      ...
      {isOpen ? (
        <div className="mt-4 grid gap-4 border-t border-white/50 pt-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-h-6 text-sm" aria-live="polite">
              {deleteState.message ? (
                <p
                  className={deleteState.status === "success" ? "text-emerald-700" : "text-rose-600"}
                >
                  {deleteState.message}
                </p>
              ) : null}
...
export function CustomersWorkspace({
  ...
  const groupedCustomers = groupNamedItemsByAlphabet(filteredCustomers);
        ...
        {filteredCustomers.length === 0 ? (
          ...
          <div className="grid gap-6">
            {groupedCustomers.map((group) => (
              <section key={group.letter} className="grid gap-3">
                <div className="flex items-center gap-3 px-1">
                  <div className="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-[linear-gradient(145deg,_rgba(122,31,75,0.92),_rgba(225,131,162,0.88))] text-sm font-semibold uppercase tracking-[0.18em] text-white shadow-[0_4px_12px_rgba(225,29,72,0.22),inset_0_2px_4px_rgba(255,255,255,0.35)]">
                    {group.letter}
                  </div>
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-zinc-500">{group.items.length} cliente(s)</p>
                  </div>
                </div>
